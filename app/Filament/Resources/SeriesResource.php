...
class SeriesResource extends Resource
{
            ...
            Section::make("Series Info")
                ->schema([
                    TextInput::make("series_name")
                        ...
                        ->afterStateUpdated(
                            fn(Set $set, ?string $state) => !empty($state)
                                ? $set(
                                    "series_slug",
                                    SlugFormatter::formatSlug($state),
                                )
                                : $set("series_slug", ""),
                        ),
                    TextInput::make("series_slug")->required()->disabled(),
                    Select::make("canon_id")->relationship("canon", "name"),
                    ...
                    Textarea::make("series_description")
                        ...
                        ->required()
                        ->maxLength(2000),
                    Toggle::make("brc_series")->label("BRC Series")->inline(),
                ])
                ->columns(2),
        ...
        return $table
            ->columns([
                TextColumn::make("series_name")
                    ...
                    ->sortable()
                    ->searchable(),
                TextColumn::make("series_slug")->label("Series Slug"),
                IconColumn::make("brc_series")->label("BRC Series")->boolean(),
            ])
            ->filters([
                //
            ])
    ...
    {
        return [BooksRelationManager::class];
    }
